...
if __name__ == '__main__':
    ...
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv1_2)))
    ...
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv2_1)))

    W_conv2_2 = tf.get_variable('conv2_2', shape=[3, 3, 256, 256], initializer=tf.contrib.keras.initializers.he_normal())
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv2_2)))
    ...
    output = tf.nn.relu(batch_norm(conv2d(output,W_conv3_1)))

    W_conv3_2 = tf.get_variable('conv3_2', shape=[3, 3, 512, 512], initializer=tf.contrib.keras.initializers.he_normal())
    output = tf.nn.relu(batch_norm(conv2d(output, W_conv3_2)))
    ...
    with tf.Session() as sess:
        ...
        conv1 = np.array(W_conv1_1.eval())
        conv2 = np.array(W_conv1_2.eval())
        conv3 = np.array(W_conv2_1.eval())
        conv4 = np.array(W_conv2_2.eval())
        conv5 = np.array(W_conv3_1.eval())
        conv6 = np.array(W_conv3_2.eval())
        np.save('cifar10/conv1.npy',conv1)
        np.save('cifar10/conv2.npy',conv2)
        np.save('cifar10/conv3.npy',conv3)
        np.save('cifar10/conv4.npy',conv4)
        np.save('cifar10/conv5.npy',conv5)
        np.save('cifar10/conv6.npy',conv6)

        save_path = saver.save(sess, model_save_path)
        print("Model saved in file: %s" % save_path)  
        saver.save(sess,save_path='cifar10-0.ckpt')
